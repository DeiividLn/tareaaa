/*jshint -W065 */
var epicconstructionRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['epic-construction-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
		...
		// The current value set in Control Class (set in Epic_Construction::to_json() function)
	...
	setImageInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.epic-construction-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.epic-construction-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ){
		...
		$targetDiv.find( '.epic-construction-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ){
		...
		$targetDiv.find( '.epic-construction-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ){
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Epic_Construction_Control_Repeater::render_content() ).
		...
		if ( template ) {
			...
			newRow = new epicconstructionRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'epic_construction_get_fontawesome_ajax',
					epic_construction_customize_nonce: epic_construction_customize.nonce
				},
				beforeSend: function(){
