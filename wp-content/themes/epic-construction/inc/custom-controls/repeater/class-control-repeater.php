<?php
/**
 * Epic_Construction Customizer Repeater Control.
 * 
 * @package Epic_Construction
*/
...
if( ! class_exists( 'Epic_Construction_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Epic_Construction_Control_Repeater extends WP_Customize_Control {
    	 ...
    	 * @var string
    	 */
    	public $type = 'epic-construction-repeater';
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'epic-construction' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'epic-construction' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
            ...
            wp_enqueue_script( 'epic-construction-repeater', get_stylesheet_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'epic-construction-repeater', get_stylesheet_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'epic-construction' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'epic-construction' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="epic-construction-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'epic-construction' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_attr_e( 'No Image Selected', 'epic-construction' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'epic-construction' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'epic-construction' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'epic-construction' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change Image', 'epic-construction' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add Image', 'epic-construction' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="epic-construction-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'epic-construction' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_attr_e( 'No File Selected', 'epic-construction' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'epic-construction' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'epic-construction' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change File', 'epic-construction' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add File', 'epic-construction' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'epic-construction' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
